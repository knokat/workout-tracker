...
function App(){
  ...
  const gL=d=>[...all].reverse().find(w=>w.day===d&&w.date>=PLAN_V7_DATE);
  const gLold=d=>[...all].reverse().find(w=>w.day===d&&w.date<PLAN_V7_DATE);
  const planLabel=(w)=>w.date<PLAN_V2_DATE?(PLAN_V1_LABELS[w.day]||PLANS[w.day]?.label):w.date<PLAN_V4_DATE?(PLAN_V2_LABELS[w.day]||PLANS[w.day]?.label):w.date<PLAN_V7_DATE?(PLAN_V6_LABELS[w.day]||PLANS[w.day]?.label):(PLAN_V7_LABELS[w.day]||PLANS[w.day]?.label);
